...
SAMPLE_PHASE_DIR=./lease_hardware_sampler/sampling_files/logs_08102020

SAMPLE_ALL=./lease_hardware_sampler/sampling_files/float_no_scope_leases
SAMPLE_ALL_PHASE=./lease_hardware_sampler/sampling_files/poly_small_scope_float
...
all_bench = 2mm 3mm adi atax bicg cholesky correlation covariance deriche doitgen durbin fdtd-2d floyd-warshall gemm gemver gesummv gramschmidt heat-3d jacobi-1d jacobi-2d lu ludcmp mvt nussinov seidel-2d symm syr2k syrk trisolv trmm
...
phase_bench = mvt#2mm 3mm atax doitgen floyd-warshall mvt nussinov
...
run_rand_with_phase_set:
	$(foreach sample, $(phase_bench), $(BIN_DIR)/rl_set $(SAMPLE_PHASE_DIR)/$(sample)_sample_log.txt 8 > $(RESULT_DIR)/$(sample)_tag_phase_leases.txt ;)
...
run_all:
	$(foreach name, $(all_bench), $(BIN_DIR)/rl_set $(SAMPLE_ALL)/poly_small_float_$(name).txt $(WAYS) > $(RESULT_DIR)/$(name)_fnsl_leases ;)
	$(foreach name, $(all_bench), $(BIN_DIR)/rl_set $(SAMPLE_ALL_PHASE)/$(name).txt $(WAYS) > $(RESULT_DIR)/$(name)_fsl_leases ;)
